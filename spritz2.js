...
function spritz(){
    ...
    var wpm = parseInt(document.getElementById("wpm").value, 10);
    if(wpm < 1){
        return;
    }
    ...
    if(selection){
        spritzify(wpm, selection);
    } else {
        spritzify(wpm, document.getElementById(song).textContent);
    }
}

function splitJapaneseText(input) {
 input = input.replace(/[\r\n]+/g, "");
...
// The meat!
function spritzify(wpm, input){


    var ms_per_word = wpm;

    // Split on any spaces.
    var all_words = splitJapaneseText(input);
/*
...
*/
    ...
    function startSpritz() {
        stopSpritz();
        var t = function() {
          spritz_timers.push(setTimeout(function() {
            //console.log("begin", new Date().getSeconds());
